...
export default function Card() {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get("https://api.joinclamp.com/v1/indexes").then((response) => {
            ...
            <div
              ...
              onClick={() => {
                ...
                dispatch(
                  setType({
                    name: e.indexName,
                    composition: e.indexComposition,
                  })
                );
              }}
            >
              <div className="inner-card">
                <div className="card-upper">
                  <div className="card-top">
                    <h2>
                      {e.indexName} <MdOutlineArrowForwardIos />
                    </h2>
                  </div>
                      ...
                      <div className="icons">
                        {e.indexComposition.map((p, k) => {
                          return (
                            <img
                              key={k}
                              src={`https://test.joinclamp.com/tokens/${p.tokenName}.svg`}
                              alt=""
                            />
                          );
                        })}
                ...
                <div className="card-bot">{/* <button>Buy</button> */}</div>
              </div>
            </div>
          );
        })
